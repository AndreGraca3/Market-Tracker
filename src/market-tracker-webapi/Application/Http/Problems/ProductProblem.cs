...
) : Problem(status, subType, title, detail, data)
{
    ...
    public static ProductProblem FromServiceError(IProductError error)
        => error switch
        {
            ProductFetchingError.ProductByIdNotFound productByIdNotFound => new ProductByIdNotFound(productByIdNotFound),
            ProductFetchingError.OutOfStockInStore outOfStockInStore => new OutOfStockInStore(outOfStockInStore),
            ProductFetchingError.ProductNotFoundInStore productNotFoundInStore => new ProductNotFoundInStore(productNotFoundInStore),
